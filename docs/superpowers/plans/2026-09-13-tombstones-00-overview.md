...
- **For slice 5 / ADR 0021:** git: a crash between writing a tombstone to the working tree and committing it leaves the tombstone uncommitted. Local reads see it, but push does not carry it, and a later `git_pull` whose forced checkout resets the working tree would discard it and resurrect the record. The same window already existed for `put`. Slice 5 should account for this when it changes the pull path. Accounted for in slice 5: `git_pull` takes `lock_repo`; the crash window is tracked in #283 and recorded in spec §3.5.
- **For slice 6:** fs: `delete`/`purge` of a key that was never stored takes the per-record lock, which creates the `<ns>/<col>/` directories and a `<id>.json.lock` file. `collect_keys` ignores lock files, but `reset` and `collect` over many absent keys will leave these behind; slice 6 may want to prune empty collection directories and stale lock files. **Decided in slice 6 (Q5(a)): not pruned; unlinking a held lock file breaks flock exclusion.**
...
- Carried from slice 5's final review, for slice 7:
  - **ADR 0021 or the guide:** callers must not push a git store while `PullReport.conflicts` is non-empty. A delete-vs-edit conflict keeps local in the merge commit, so a push would publish the live record over the remote's tombstone.
  - **CHANGELOG 0.7.0:**
    - `SyncReport` gains `fast_forwarded_to_a` / `fast_forwarded_to_b`.
    - A record behind the other side's chain is now reported as fast-forwarded, not `merged`.
    - `gonzalo sync` prints `fast_forwarded: N`.
    - The CLI sync labels are no longer column-aligned; re-align them once, with a note.
  - **Follow-up issues filed at slice 5 ship:** the pull ancestor-ordering gap, and the `SyncReport` non-convergence signal. Refer to them as "filed at slice 5 ship" (no issue numbers yet).
- Carried from slice 6's final review, for slice 7:
  - (a) **Authorship correction for the three slice 7 plan lines.** CLI `delete` and `reset` both stamp `gonzalo-cli`; library `reset` keeps each record's own author; `reset_as` names a deleter; over a daemon spec §3.6's claimed-deleter rule applies.
  - (b) **Spec §3.7** needs a sentence on `reset_as`.
  - (c) **Spec §3.10** needs "(if the CLI ever opens a `ServerStore`, §3.6's claimed-deleter rule applies)" (Ruling Q1(a)).
  - (d) **Mem0 `reset` row 🟡 → ✅** citing ADR 0021 (final-review I2).
  - (e) **Guide notes** on the Q5(a) lock-file and directory residue, including `delete` under a mistyped `--root` creating directories, and on git's one commit per reset or collect key.
  - (f) **CHANGELOG entries:** the `delete`/`reset`/`collect` commands, exit code 3, `sync --ancestor-cap`, and `gonzalo_core::{reset, reset_as, collect, ResetReport, CollectReport}`.
  - (g) **The slice 6 plan text is superseded by the ledger rulings.** Its branch name, the `exits_one` test name, `reset` rather than `reset_as` in the CLI wrapper, and the "no guide page" note are stale; slice 7 consumes the revised slice 6 plan.
  - **Closing #203:** slice 6 does not close #203, because criterion 4 needs ADR 0021. Slice 7's PR ticks the Mem0 matrix `reset` whole-namespace row (`docs/evaluation/competitors/mem0/parity-gap-matrix.md`) 🟡 → ✅ citing ADR 0021, alongside the Delete row, and its body says `Closes #203`.
  - **Parked final-review Minors:** m2 (the `gonzalo` facade re-exports `reset`, `reset_as`, `collect`, `ResetReport` and `CollectReport`; add to CHANGELOG), m3 (the guide's `--expected` wording should say the `revision` object from `gonzalo get`'s JSON), and m7 (a mistyped `--root` creates lock-file directories and still exits 0; add a guide note).
